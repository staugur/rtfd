...
_env_manager() {
    ...
    *)
    ...
    cat >>$sphinx_conf <<EOF
#: Automatic generated by rtfd in $(date '+%Y-%m-%d %H:%M:%S')
if not 'html_js_files' in globals():
    html_js_files = []
html_js_files.append(("${server_static_url}rtfd.js?v=$(rtfd -v)",{"id":"rtfd-script","data":"rtfd_api=${rtfd_server}&rtfd_static=${server_static_url}&name=${project_name}&branch=${branch}"}))
