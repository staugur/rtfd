...
# 固定多版本 Python：基础镜像自带 python3(=3.12)；通过 deadsnakes PPA 安装 3.10 / 3.11，
# 各版本独立落在 /usr/bin/python3.X，互不冲突（相比 uv 动态安装更可控、可复现）。
# 版本列表如需调整，改下面两处 apt 安装 + assets/rtfd.cfg 的 [py] 分区即可。
RUN apt-get update -y && \
    apt-get install -y --no-install-recommends \
        software-properties-common ca-certificates curl git procps \
...
# 为 deadsnakes 的 3.10 / 3.11 各自装入 virtualenv 模块（系统 python3 的 virtualenv 由上面的
