...
写回数据库元数据并交由 Caddy 静态托管；支持 CLI、HTTP API、Git 仓库 Webhook、
...
运行时依赖（仅 Linux）：bash、git、python3（含 pip、virtualenv，支持配置多版本，已移除 python2）、caddy；
...
│   ├── conf/           ini 配置封装
│   ├── lib/            核心业务：lib.go(ProjectManager/CRUD) caddy.go(Caddyfile 模板) update.go(更新钩子) app.go(GitHub App)
...
| 类型 | 风格 | 示例 |
|---|---|---|
| 包/文件 | 小写单词（多词不强制） | `pkg/build`、`caddy.go` |
...
- 复杂逻辑（如 Options→Caddyfile 渲染、builder 参数回查）写清"为什么"，不逐行翻译代码
...
- 对外部命令/网络调用（git、caddy、GitHub API）须检查 error 与 exit code
...
- 一律通过 `conf.New(path)` 后使用 `GetKey/SecHash/GetPath/BaseDir/MustPath` 等方法，
  ...
  与 `main_test.go` 断言
- Caddyfile 模板（`pkg/lib/caddy.go`）的缓存开关来自 `[caddy]` 分区的
  `static_expires / html_nocache`，由 `renderCaddy` 读取；
  **改动模板需同步 `pkg/lib/caddy_test.go` 中的指令与缓存断言**
